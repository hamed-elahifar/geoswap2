...
describe("Token contract", () => {
  ...
  before(async () => {
    ...
    TokenA = await ethers.getContractFactory("A");
    tokenA = await TokenA.deploy();

    TokenB = await ethers.getContractFactory("B");
    tokenB = await TokenB.deploy();

    TokenC = await ethers.getContractFactory("C");
    tokenC = await TokenC.deploy();

    WETH = await ethers.getContractFactory("WETH");
    weth = await WETH.deploy();

    Router02 = await ethers.getContractFactory("SolarRouter02");
  ...
  describe("Deployment", () => {
    ...
    xit("should add Liquidity to A-B Pair", async () => {
    ...
    xit("should add Liquidity to B-C Pair", async () => {
      const amountB = (await tokenB.balanceOf(owner1.address)) / 10 ** 18;
    ...
    xit("should swap from A to B", async () => {
